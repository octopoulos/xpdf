...
GString* appendToPath(GString* path, const char* fileName)
{
	...
	if (path->getLength() > 0 && path->getChar(path->getLength() - 1) != '/' && path->getChar(path->getLength() - 1) != '\\')
		path->append('/');
	path->append(fileName);
	return path;
...
void readWindowsShortcut(wchar_t* wPath, size_t wPathSize)
{
	...
	HRESULT      hres;
	hres              = CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, IID_IShellLinkW, (LPVOID*)&shellLink);
