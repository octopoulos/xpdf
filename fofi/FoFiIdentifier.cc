...
static FoFiIdentifierType identify(Reader* reader)
{
	...
	//----- TrueType
	if ((reader->getByte(0) == 0x00 && reader->getByte(1) == 0x01 && reader->getByte(2) == 0x00 && reader->getByte(3) == 0x00) || (reader->getByte(0) == 0x74 && // 'true'
...
static FoFiIdentifierType identifyCFF(Reader* reader, int start)
{
	...
	if ((offSize1 = reader->getByte(pos + 2)) < 1 || offSize1 > 4)
		return fofiIdUnknown;
	if (!reader->getUVarBE(pos + 3, offSize1, &offset0) || offset0 > (Guint)INT_MAX || !reader->getUVarBE(pos + 3 + offSize1, offSize1, &offset1) || offset1 > (Guint)INT_MAX || offset0 > offset1)
		return fofiIdUnknown;
