...
void FoFiType1C::convertToType1(char* psName, const char** newEncoding, GBool ascii, FoFiOutputFunc outputFunc, void* outputStream)
{
	...
	buf = GString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] readonly def\n", topDict.fontMatrix[0], topDict.fontMatrix[1], topDict.fontMatrix[2], topDict.fontMatrix[3], topDict.fontMatrix[4], topDict.fontMatrix[5]);
	(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	delete buf;
...
void FoFiType1C::convertToCIDType0(char* psName, int* codeMap, int nCodes, FoFiOutputFunc outputFunc, void* outputStream)
{
	...
	for (i = 0; i < nCIDs; ++i)
	{
		...
		if ((gid = cidMap[i]) >= 0)
		{
			...
			if (ok)
			{
				...
				if (!ok)
					subrIdx.pos = -1;
				cvtGlyph(val.pos, val.len, charStrings, &subrIdx, &privateDicts[fdSelect ? fdSelect[gid] : 0], gTrue, 0);
			}
		}
	}
	...
	for (i = 0; i < nFDs; ++i)
	{
		...
		{
			buf = GString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] def\n", privateDicts[i].fontMatrix[0], privateDicts[i].fontMatrix[1], privateDicts[i].fontMatrix[2], privateDicts[i].fontMatrix[3], privateDicts[i].fontMatrix[4], privateDicts[i].fontMatrix[5]);
			(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
			delete buf;
		}
		else
...
void FoFiType1C::convertToType0(char* psName, int* codeMap, int nCodes, FoFiOutputFunc outputFunc, void* outputStream)
{
	...
	for (i = 0; i < nCIDs; i += 256)
	{
		...
		{
			buf = GString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] def\n", privateDicts[fd].fontMatrix[0], privateDicts[fd].fontMatrix[1], privateDicts[fd].fontMatrix[2], privateDicts[fd].fontMatrix[3], privateDicts[fd].fontMatrix[4], privateDicts[fd].fontMatrix[5]);
			(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
			delete buf;
		}
...
void FoFiType1C::cvtGlyph(int offset, int nBytes, GString* charBuf, Type1CIndex* subrIdx, Type1CPrivateDict* pDict, GBool top, int recursion)
{
	Type1CIndexVal val;
	Type1COp       zero, tmp;
	GBool          ok, dInt;
		...
		if (ops[nOps - 1].kind == type1COpOperator)
		{
			...
			case 0x000a: // callsubr
				if (nOps >= 1)
				{
					...
					if (ok)
						cvtGlyph(val.pos, val.len, charBuf, subrIdx, pDict, gFalse, recursion + 1);
				}
				else
				{
...
void FoFiType1C::convertToOpenType(FoFiOutputFunc outputFunc, void* outputStream, int nWidths, Gushort* widths, Guchar* cmapTable, int cmapTableLen)
{
