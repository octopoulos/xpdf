...
static ArgDesc argDesc[] = {
	...
	{ "-vstretch", argFP, &vStretch, 0,     "vertical stretch factor (1.0 means no stretching)" },
	{ "-embedbackground", argFlag, &embedBackground, 0,     "embed the background image as base64-encoded data" },
	{ "-nofonts", argFlag, &noFonts, 0,     "do not extract embedded fonts" },
	{ "-embedfonts", argFlag, &embedFonts, 0,     "embed the fonts as base64-encoded data" },
	...
	{ "-formfields", argFlag, &formFields, 0,     "convert form fields to HTML" },
	{ "-meta", argFlag, &includeMetadata, 0,     "include document metadata in the HTML output" },
	{ "-table", argFlag, &tableMode, 0,     "use table mode for text extraction" },
	{ "-overwrite", argFlag, &overwrite, 0,     "overwrite files in an existing output directory" },
	...
	try
	{
		...
		if (makeDir(htmlDir, 0755))
		{
			if (pathIsDir(htmlDir))
			{
				if (!overwrite)
				{
					error(errIO, -1, "HTML output directory '{0:s}' already exists (use '-overwrite' to overwrite it)", htmlDir);
					exitCode = 2;
					goto err1;
				}
			}
		...
		for (pg = firstPage; pg <= lastPage; ++pg)
		{
			...
			else if (!(pngFile = openFile(pngFileName->getCString(), "wb")))
			{
			...
			err    = htmlGen->convertPage(pg, pngURL->getCString(), htmlDir, &writeToFile, htmlFile, &writeToFile, pngFile);
			delete pngURL;
			fclose(htmlFile);
			if (!embedBackground)
				fclose(pngFile);
