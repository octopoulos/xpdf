...
Operator Gfx::opTab[] = {
	...
	{ "SC",  -4,  { tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum }, &Gfx::opSetStrokeColor     },
	{ "SCN", -33, { tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN }, &Gfx::opSetStrokeColorN    },
	...
	{ "sc",  -4,  { tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum }, &Gfx::opSetFillColor       },
	{ "scn", -33, { tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN }, &Gfx::opSetFillColorN      },
...
GfxFont* GfxResources::lookupFont(char* name)
{
	...
	error(errSyntaxError, -1, "Unknown font tag '{}'", name);
	return nullptr;
}
...
GfxFont* GfxResources::lookupFontByRef(Ref ref)
{
	...
	error(errSyntaxError, -1, "Unknown font ref {}.{}", ref.num, ref.gen);
	return nullptr;
}
...
bool GfxResources::lookupXObject(const char* name, Object* obj)
{
	...
	error(errSyntaxError, -1, "XObject '{}' is unknown", name);
	return false;
}
...
void GfxResources::lookupColorSpace(const char* name, Object* obj, bool inherit)
{
	GfxResources* resPtr;
...
GfxPattern* GfxResources::lookupPattern(const char* name)
{
	...
	error(errSyntaxError, -1, "Unknown pattern '{}'", name);
	return nullptr;
}
...
GfxShading* GfxResources::lookupShading(const char* name)
{
	...
	error(errSyntaxError, -1, "Unknown shading '{}'", name);
	return nullptr;
}
...
bool GfxResources::lookupGState(const char* name, Object* obj)
{
	...
	error(errSyntaxError, -1, "ExtGState '{}' is unknown", name);
	return false;
}
...
bool GfxResources::lookupPropertiesNF(const char* name, Object* obj)
{
	...
	error(errSyntaxError, -1, "Properties '{}' is unknown", name);
	return false;
}
...
bool Gfx::execOp(Object* cmd, Object args[], int numArgs)
{
	...
	if (!(op = findOp(name)))
	{
		if (ignoreUndef > 0)
			return true;
		error(errSyntaxError, getPos(), "Unknown operator '{}'", name);
		return false;
	}
	...
	if (op->numArgs >= 0)
	{
		...
		{
			error(errSyntaxError, getPos(), "Too few ({}) args to '{}' operator", numArgs, name);
			return false;
		}
		if (numArgs > op->numArgs)
		{
#if 0
      error(errSyntaxWarning, getPos(), "Too many ({}) args to '{}' operator", numArgs, name);
#endif
	...
	for (i = 0; i < numArgs; ++i)
	{
		...
		{
			error(errSyntaxError, getPos(), "Arg #{} to '{}' operator is wrong type ({})", i, name, argPtr[i].getTypeName());
			return false;
		}
	}
...
void Gfx::opSetExtGState(Object args[], int numArgs)
{
	...
	if (!obj1.isDict())
	{
		error(errSyntaxError, getPos(), "ExtGState '{}' is wrong type", args[0].getName());
		obj1.free();
		return;
	}
	if (printCommands)
	...
	{
		opac = obj2.getNum();
		state->setFillOpacity(opac < 0 ? 0 : opac > 1 ? 1
		                                              : opac);
	...
	{
		opac = obj2.getNum();
		state->setStrokeOpacity(opac < 0 ? 0 : opac > 1 ? 1
		                                                : opac);
	...
	if (!obj1.dictLookup("SMask", &obj2)->isNull())
	{
		...
		else if (obj2.isDict())
		{
			...
			if (!obj2.dictLookup("TR", &obj3)->isNull())
			{
				...
				else if (!(funcs[0] = Function::parse(&obj3, 1, 1)))
				{
			...
			if (obj2.dictLookup("G", &obj3)->isStream())
			{
				if (obj3.streamGetDict()->lookup("Group", &obj4)->isDict())
				{
					...
					doSoftMask(&obj3, &objRef3, alpha, true, knockout, funcs[0], &backdropColorObj);
					objRef3.free();
					if (funcs[0])
						delete funcs[0];
				}
...
void Gfx::doSoftMask(Object* str, Object* strRef, bool alpha, bool isolated, bool knockout, Function* transferFunc, Object* backdropColorObj)
{
	Dict * dict, *resDict;
	double m[6], bbox[4];
	Object obj1, obj2;
	...
	// draw it
	++formDepth;
	drawForm(strRef, resDict, m, bbox, true, true, isolated, knockout, alpha, transferFunc, backdropColorObj);
	--formDepth;
...
void Gfx::opFillStroke(Object args[], int numArgs)
{
	...
	if (state->isPath())
	{
		if (ocState)
		{
			if (state->getFillColorSpace()->getMode() == csPattern || state->getStrokeColorSpace()->getMode() == csPattern)
			{
				if (state->getFillColorSpace()->getMode() == csPattern)
					doPatternFill(false);
				else
...
void Gfx::doPatternFill(bool eoFill)
{
	...
	case 2:
		...
		break;
	default:
		error(errSyntaxError, getPos(), "Unknown pattern type ({}) in fill", pattern->getType());
		break;
	}
}
...
void Gfx::doPatternStroke()
{
	...
	case 2:
		...
		break;
	default:
		error(errSyntaxError, getPos(), "Unknown pattern type ({}) in stroke", pattern->getType());
		break;
	}
}

void Gfx::doPatternText(bool stroke)
{
	GfxPattern* pattern;
...
void Gfx::doPatternImageMask(Object* ref, Stream* str, int width, int height, bool invert, bool inlineImg, bool interpolate)
{
	saveState();
...
void Gfx::doTilingPatternFill(GfxTilingPattern* tPat, bool stroke, bool eoFill, bool text)
{
	...
	if (out->useTilingPatternFill() && !tPat->usesBlendMode(xref))
	{
		m1[4] = m[4];
		m1[5] = m[5];
		out->tilingPatternFill(state, this, tPat->getContentStreamRef(), tPat->getPaintType(), tPat->getTilingType(), tPat->getResDict(), m1, bbox, xi0, yi0, xi1, yi1, xstep, ystep);
	}
	else
	{
		abortCheckCounter = 0;
...
void Gfx::doAxialShFill(GfxAxialShading* shading)
{
	...
	if (shading->getExtend0() && tMinExt < tMin)
	{
		...
		// compute the coordinates of the point on the t axis at t = tMinExt;
		// then compute the intersection of the perpendicular line with the bounding box
		tx = x0 + tMinExt * dx;
		ty = y0 + tMinExt * dy;
		if (dxdyZero)
		{
	...
	if (shading->getExtend1() && tMaxExt > tMax)
	{
		...
		// compute the coordinates of the point on the t axis at t = tMaxExt;
		// then compute the intersection of the perpendicular line with the bounding box
		tx = x0 + tMaxExt * dx;
		ty = y0 + tMaxExt * dy;
		if (dxdyZero)
...
void Gfx::gouraudFillTriangle(double x0, double y0, double* color0, double x1, double y1, double* color1, double x2, double y2, double* color2, GfxGouraudTriangleShading* shading, int depth)
{
	...
	if (i == nComps || depth == gouraudMaxDepth || (fabs(dx0) < 0.5 && fabs(dy0) < 0.5 && fabs(dx1) < 0.5 && fabs(dy1) < 0.5 && fabs(dx2) < 0.5 && fabs(dy2) < 0.5))
	{
...
void Gfx::doShowText(const std::string& s)
{
	...
	else if (out->useDrawChar())
	{
		...
		while (len > 0)
		{
			...
			out->drawChar(state, state->getCurX() + riseX, state->getCurY() + riseY, tdx, tdy, tOriginX, tOriginY, code, n, u, uLen, doFill, doStroke, doMakePath);
			state->shift(tdx, tdy);
			p += n;
			len -= n;
		}
	...
	else
	{
		...
		while (len > 0)
		{
			...
			int      n = font->getNextChar(p, len, &code, u, (int)(sizeof(u) / sizeof(Unicode)), &uLen, &dx2, &dy2, &originX, &originY);
			dx += dx2;
			dy += dy2;
			if (n == 1 && *p == ' ')
				++nSpaces;
...
void Gfx::doIncCharCount(const std::string& s)
{
	if (out->needCharCount())
		out->incCharCount(TO_INT(s.size()));
}
...
void Gfx::opXObject(Object args[], int numArgs)
{
	...
	if (!xObj.isStream())
	{
		error(errSyntaxError, getPos(), "XObject '{}' is wrong type", name);
		xObj.free();
		refObj.free();
		return;
	}
	...
	try
	{
		...
		{
			if (out->needNonText())
				doImage(&refObj, xObj.getStream(), false);
		}
		...
		else if (obj2.isName("PS"))
		{
			if (ocState)
			{
				...
				out->psXObject(xObj.getStream(), obj3.isStream() ? obj3.getStream() : (Stream*)nullptr);
				obj3.free();
			}
		}
		else if (obj2.isName())
		{
			error(errSyntaxError, getPos(), "Unknown XObject subtype '{}'", obj2.getName());
		}
		else
		{
...
bool Gfx::doImage(Object* ref, Stream* str, bool inlineImg)
{
	Dict *               dict, *maskDict;
	int                  width, height;
	int                  bits, maskBits;
	...
	bool                 haveColorKeyMask, haveExplicitMask, haveSoftMask, haveMatte;
	...
	if (bits == 0)
	{
		...
		if (obj1.isNum())
		{
			...
			{
				error(errSyntaxWarning, getPos(), "Non-integer image bits per component");
	...
	else
	{
		...
		// get the mask
		haveColorKeyMask = haveExplicitMask = haveSoftMask = haveMatte = false;
		maskStr                                                        = nullptr; // make gcc happy
		maskWidth = maskHeight = 0;                                               // make gcc happy
		maskInvert             = false;                                           // make gcc happy
		maskColorMap           = nullptr;                                         // make gcc happy
		...
		if (smaskObj.isStream())
		{
			...
			{
				error(errSyntaxWarning, getPos(), "Non-integer image mask bits per component");
		...
		else if (haveSoftMask)
		{
			dict->lookupNF("SMask", &maskRef);
			out->drawSoftMaskedImage(state, ref, str, width, height, colorMap, &maskRef, maskStr, maskWidth, maskHeight, maskColorMap, haveMatte ? matte : (double*)nullptr, interpolate);
			maskRef.free();
			delete maskColorMap;
		}
		else if (haveExplicitMask)
		{
			...
			out->drawMaskedImage(state, ref, str, width, height, colorMap, &maskRef, maskStr, maskWidth, maskHeight, maskInvert, interpolate);
			maskRef.free();
		}
		else
		{
			out->drawImage(state, ref, str, width, height, colorMap, haveColorKeyMask ? maskColors : (int*)nullptr, inlineImg, interpolate);
		}
...
void Gfx::doForm(Object* strRef, Object* str)
{
	...
	// draw it
	++formDepth;
	drawForm(strRef, resDict, m, bbox, transpGroup, false, isolated, knockout);
	--formDepth;

	resObj.free();
}

void Gfx::drawForm(Object* strRef, Dict* resDict, double* matrix, double* bbox, bool transpGroup, bool softMask, bool isolated, bool knockout, bool alpha, Function* transferFunc, Object* backdropColorObj)
{
	Parser*        oldParser;
	GfxState*      savedState;
	...
	if (softMask || transpGroup)
	{
		...
		if (!out->beginTransparencyGroup(state, bbox, blendingColorSpace, isolated, knockout, softMask))
			transpGroup = false;
	...
	if (softMask)
	{
		...
		{
			for (i = 0; i < backdropColorObj->arrayGetLength() && i < gfxColorMaxComps; ++i)
			{
...
Stream* Gfx::buildImageStream(bool* haveLength)
{
	Object  dict;
...
void Gfx::opBeginMarkedContent(Object args[], int numArgs)
{
	...
	mcKind = gfxMCOther;
	if (args[0].isName("OC") && numArgs == 2 && args[1].isName() && res->lookupPropertiesNF(args[1].getName(), &obj))
	{
	...
	else if (numArgs == 2 && args[1].isDict())
	{
		...
		{
			out->beginStructureItem(args[0].getName(), obj.getInt(), args[1].getDict());
...
{
	GfxResources* resPtr = res->getNext();
	delete res;
	res = resPtr;
}
