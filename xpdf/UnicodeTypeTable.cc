...
bool unicodeTypeCombiningMark(Unicode c)
{
	return getType(c) == 'M';
}

bool unicodeTypeDigit(Unicode c)
{
	return getType(c) == '#';
}

bool unicodeTypeNumSep(Unicode c)
{
	return getType(c) == '.';
}
...
bool unicodeBracketInfo(Unicode c, bool* open, Unicode* opposite)
{
	Unicode bracket;

	if (c > 0xffff) return false;
	const int i = (c >> 8) & 0xff;
	if (!bracketTable[i]) return false;
