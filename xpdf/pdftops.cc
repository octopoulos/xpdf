...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		if ((level1 ? 1 : 0) + (level1Sep ? 1 : 0) + (level2 ? 1 : 0) + (level2Gray ? 1 : 0) + (level2Sep ? 1 : 0) + (level3 ? 1 : 0) + (level3Gray ? 1 : 0) + (level3Sep ? 1 : 0) > 1)
		{
		...
		doc->displayPages(psOut, firstPage, lastPage, 72, 72, 0, !globalParams->getPSUseCropBoxAsPage(), globalParams->getPSCrop(), gTrue);
		exitCode = 0;
		if (!psOut->checkIO())
			exitCode = 2;
		delete psOut;
