...
GList* TileMap::getTileList()
{
	...
	case displaySingle:
		...
		for (y = y0 / tileH1; y <= y1 / tileH1; ++y)
		{
			for (x = x0 / tileW1; x <= x1 / tileW1; ++x)
			{
				...
				if (ty + th > pageH1)
					th = pageH1 - ty;
				tiles->append(new PlacedTileDesc(page, state->getRotate(), pageDPI1, tx, ty, tw, th, tx - state->getScrollX() + offsetX, ty - state->getScrollY() + offsetY));
			}
		}
		break;
	...
	case displayContinuous:
		...
		while (page <= state->getDoc()->getNumPages() && pageY[page - 1] < state->getScrollY() + state->getWinH())
		{
			pageDPI1 = pageDPI[page - 1];
			pageW1   = pageW[page - 1];
			pageH1   = pageH[page - 1];
		...
		if (page + 1 <= state->getDoc()->getNumPages())
		{
			...
			for (y = y0 / tileH2; y <= y1 / tileH2; ++y)
			{
				for (x = x0 / tileW2; x <= x1 / tileW2; ++x)
				{
					...
					if (ty + th > pageH2)
						th = pageH2 - ty;
					tiles->append(new PlacedTileDesc(page + 1, state->getRotate(), pageDPI2, tx, ty, tw, th, tx - state->getScrollX() + offsetX2, ty - state->getScrollY() + offsetY));
				}
			}
		}
		break;
	...
	case displaySideBySideContinuous:
		...
		while (page <= state->getDoc()->getNumPages() && (pageY[page - 1] < state->getScrollY() + state->getWinH() || (page + 1 <= state->getDoc()->getNumPages() && pageY[page] < state->getScrollY() + state->getWinH())))
		{
			pageDPI1 = pageDPI[page - 1];
			pageW1   = pageW[page - 1];
			pageH1   = pageH[page - 1];
	...
	case displayHorizontalContinuous:
		...
		while (page <= state->getDoc()->getNumPages() && pageX[page - 1] < state->getScrollX() + state->getWinW())
		{
			pageDPI1 = pageDPI[page - 1];
			pageW1   = pageW[page - 1];
			pageH1   = pageH[page - 1];
