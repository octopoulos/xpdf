...
{
public:
	// Parse a CMap from <obj>, which can be a name or a stream.
	// Sets the initial reference count to 1.  Returns nullptr on failure.
	static CMap* parse(CMapCache* cache, const std::string& collectionA, Object* obj);

	// Create the CMap specified by <collection> and <cMapName>.
	...
	// Return true if this CMap matches the specified <collectionA>, and <cMapNameA>.
	bool match(const std::string& collectionA, const std::string& cMapNameA);

	// Return the CID corresponding to the character code starting at <s>, which contains <len> bytes.
	// Sets *<c> to the char code, and *<nUsed> to the number of bytes used by the char code.
...
private:
	...
	std::string      cMapName   = "";      //
	bool             isIdent    = false;   // true if this CMap is an identity mapping, or is based on one (via usecmap)
	int              wMode      = 0;       // writing mode (0=horizontal, 1=vertical)
	CMapVectorEntry* vector     = nullptr; // vector for first byte (nullptr for  identity CMap)
...
public:
	...
	// Increments its reference count; there will be one reference for the cache plus one for the caller of this function.
	// Returns nullptr on failure.
