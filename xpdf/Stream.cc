...
{
	if (pred)
		return new LZWStream(str->copy(), pred->getPredictor(), pred->getWidth(), pred->getNComps(), pred->getNBits(), early);
	else
...
void DCTStream::reset()
{
	...
	sourceMgr.src.next_input_byte   = nullptr;
...
void DCTStream::reset()
{
	...
	if (colorXform == -1)
	{
		if (numComps == 3)
		{
			...
			else if (compInfo[0].id == 82 && compInfo[1].id == 71 && compInfo[2].id == 66)
			{ // ASCII "RGB"
				colorXform = 0;
			}
			else
...
void DCTStream::prepare()
{
	...
	if (progressive || !interleaved)
	{
		...
		if (bufWidth <= 0 || bufHeight <= 0 || bufWidth > INT_MAX / bufHeight / (int)sizeof(int))
		{
...
void DCTStream::readScan()
{
	...
	for (cc = 0; cc < numComps; ++cc)
	{
		if (scanInfo.comp[cc] && (scanInfo.dcHuffTable[cc] >= numDCHuffTables || ((!progressive || scanInfo.lastCoeff > 0) && scanInfo.acHuffTable[cc] >= numACHuffTables)))
		{
			error(errSyntaxError, getPos(), "Bad DCT data: invalid Huffman table index");
			return;
		}
...
{
	if (pred)
		return new FlateStream(str->copy(), pred->getPredictor(), pred->getWidth(), pred->getNComps(), pred->getNBits());
	else
