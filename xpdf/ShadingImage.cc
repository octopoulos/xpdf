...
SplashBitmap* ShadingImage::generateBitmap(GfxState* state, GfxShading* shading, SplashColorMode mode, GBool reverseVideo, Splash* parentSplash, SplashBitmap* parentBitmap, int* xOut, int* yOut)
{
	...
	{
	case 1:
		return generateFunctionBitmap(state, (GfxFunctionShading*)shading, mode, reverseVideo, parentSplash, parentBitmap, xOut, yOut);
		break;
	case 2:
		return generateAxialBitmap(state, (GfxAxialShading*)shading, mode, reverseVideo, parentSplash, parentBitmap, xOut, yOut);
		break;
	case 3:
		return generateRadialBitmap(state, (GfxRadialShading*)shading, mode, reverseVideo, parentSplash, parentBitmap, xOut, yOut);
		break;
	case 4:
	case 5:
		return generateGouraudTriangleBitmap(state, (GfxGouraudTriangleShading*)shading, mode, reverseVideo, parentSplash, parentBitmap, xOut, yOut);
		break;
	case 6:
	case 7:
...
SplashBitmap* ShadingImage::generateAxialBitmap(GfxState* state, GfxAxialShading* shading, SplashColorMode mode, GBool reverseVideo, Splash* parentSplash, SplashBitmap* parentBitmap, int* xOut, int* yOut)
{
	...
	else
	{
		...
		int nColors = (int)(1.5 * sqrt((xx1 - xx0) * (xx1 - xx0) + (yy1 - yy0) * (yy1 - yy0)));
		if (nColors < 16)
			nColors = 16;
    ...
    int* xOut, int* yOut)
{
	...
	for (int i = 0; i < shading->getNTriangles(); ++i)
	{
		...
		gouraudFillTriangle(state, bitmap, mode, reverseVideo, xMin, yMin, xMax, yMax, x0, y0, color0, x1, y1, color1, x2, y2, color2, shading);
	}
    ...
    int* xOut, int* yOut)
{
	...
	for (int i = 0; i < shading->getNPatches(); ++i)
		fillPatch(state, splash, mode, reverseVideo, xMin, yMin, shading->getPatch(i), shading, start);
...
void ShadingImage::fillPatch(GfxState* state, Splash* splash, SplashColorMode mode, GBool reverseVideo, int xMin, int yMin, GfxPatch* patch, GfxPatchMeshShading* shading, int depth)
{
	...
	if (stop)
	{
		...
		path->curveTo(xx1 - xMin, yy1 - yMin, xx2 - xMin, yy2 - yMin, xx3 - xMin, yy3 - yMin);
	...
	else
	{
		...
		fillPatch(state, splash, mode, reverseVideo, xMin, yMin, &patch00, shading, depth + 1);
		fillPatch(state, splash, mode, reverseVideo, xMin, yMin, &patch10, shading, depth + 1);
		fillPatch(state, splash, mode, reverseVideo, xMin, yMin, &patch01, shading, depth + 1);
		fillPatch(state, splash, mode, reverseVideo, xMin, yMin, &patch11, shading, depth + 1);
	}
}
...
void ShadingImage::transformBBox(GfxState* state, double uxMin, double uyMin, double uxMax, double uyMax, double* dxMin, double* dyMin, double* dxMax, double* dyMax)
{
	double tx, ty;
