...
public:
	PlacedTileDesc(int pageA, int rotateA, double dpiA, int txA, int tyA, int twA, int thA, int pxA, int pyA)
	    : TileDesc(pageA, rotateA, dpiA, txA, tyA, twA, thA)
	    , px(pxA)
	    , py(pyA)
	{
	}
...
// Tile map: this maps window position to tile, and vice versa;
// and computes the list of tiles needed for the current display.
class TileMap
{
public:
	...
	// Returns a list of PlacedTileDesc objects describing the tiles needed for the current display.
	// The returned list is owned by the TileMap object (and may be reused)
	...
	// Conversions to window space return false if the specified page isn't visible (in non-continuous modes).
	// Conversions from window space return false if the point isn't on a page.
	// Otherwise, these functions do not do bounds-checking or any clipping to window or page coordinates.
	bool cvtWindowToUser(int xw, int yw, int* pg, double* xu, double* yu);
	bool cvtWindowToDev(int xw, int yw, int* pg, int* xd, int* yd);
	bool cvtUserToWindow(int pg, double xu, double yu, int* xw, int* yw);
	...
	// Return the range of pages intersected by a rectangle in window coordinates.
	// E.g., these are the pages that will need to be redrawn for a redraw event covering that rectangle.
	void getWindowPageRange(int x, int y, int w, int h, int* firstPage, int* lastPage);

	// Return the scrollY setting that would place the top of <page> at the top of the window.
	// (In non-continuous modes, this just returns zero.)
	int getPageTopY(int page);

	// Return the scrollY setting that would place the bottom of <page> at the bottom of the window.
	int getPageBottomY(int page);

	// Return the scrollX setting that would place the left side of <page> at the left side of the window.
	// (In non-continuous modes, this just returns zero.)
	int getPageLeftX(int page);

	// Return the scrollX setting that would place the right side of <page> at the right side of the window.
	int getPageRightX(int page);

	// In continuous modes: return the lowest page number visible anywhere in the window;
	// in non-continuous modes: return the current page.
	int getFirstPage();

	// In continuous modes: return the lowest page number whose top (or left) edge is visible in the window
	// (or if no top/left edge is visilble, the page after the single visible page);
	// in non-continuous modes: return the current page.
	int getFirstPageTop();

	// In continuous modes: return the page number at the center of the window;
	// in non-continuous modes: return the current page.
	int getMidPage();

	// In continuous modes: return the highest page number visible anywhere in the window;
	// in non-continuous modes: return the current page.
	int getLastPage();
	...
	// Get the number of pixels (vertically) between adjacent pages in continuous / side-by-side continuous mode.
	int getContinuousPageSpacing();

	// Get the number of pixels (horizontally) between facing pages in the side-by-side modes.
	int getSideBySidePageSpacing();

	// Get the number of pixels (horizontally) between adjacent pages in horizontal continuous mode.
...
private:
	...
	// Compute the user-to-device transform matrix for the specified page.
	void computePageMatrix(int page, double* m);

	// Invert a matrix.
	void invertMatrix(double* m, double* im);

	// Return the page number at y.
	// If y falls in a gap between pages, return the next page.
	// If y falls after the last page, return numPages + 1.
	int findContinuousPage(int y);

	// Return the left-side page number at y.
	// If y falls in a gap between page pairs, return the next page.
	// If y falls after the last page, return numPages + 1 (or numPages + 2 if numPages is odd).
	int findSideBySideContinuousPage(int y);

	// Return the page number at x.
	// If x falls in a gap between pages, return the next page.
