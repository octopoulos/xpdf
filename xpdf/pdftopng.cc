...
static void SavePng(SplashBitmap* bitmap, int bitDepth, int colorType)
{
	...
	const char* filename = "c:/web/gori/source/pdf/xxx.png";
	...
	try
	{
		...
		for (pg = firstPage; pg <= lastPage; ++pg)
		{
			...
			{ // RGB
				...
				//writePNGData(png, splashOut->getBitmap());
				//writeRedImage(png, splashOut->getBitmap());
				//finishPNG(&png, &pngInfo);
				fclose(f);

				SavePng(splashOut->getBitmap(), 8, pngAlpha ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB);
			}
		}
		delete splashOut;
