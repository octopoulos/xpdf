	...
	while (i < path->length)
	{
		...
		else
		{
			...
			if (path->flags[i] & splashPathCurve)
			{
				...
				x3 = pts[i + 2].x;
				y3 = pts[i + 2].y;
				addCurve(x0, y0, x1, y1, x2, y2, x3, y3, flatness, (path->flags[i - 1] & splashPathFirst), (path->flags[i + 2] & splashPathLast), !closeSubpaths && (path->flags[i - 1] & splashPathFirst) && !(path->flags[i - 1] & splashPathClosed), !closeSubpaths && (path->flags[i + 2] & splashPathLast) && !(path->flags[i + 2] & splashPathClosed));
				x0 = x3;
				y0 = y3;
				i += 3;
			...
			{
				if (closeSubpaths && (pts[i - 1].x != pts[curSubpath].x || pts[i - 1].y != pts[curSubpath].y))
...
bool SplashXPath::strokeAdjust(SplashXPathPoint* pts, SplashPathHint* hints, int nHints, SplashStrokeAdjustMode strokeAdjMode, SplashClip* clip)
{
	...
	for (i = 0; i < nHints; ++i)
	{
		...
		if (clipTweak)
		{
			...
			if (adj0 < c0 && c0 < adj1 && adj1 < c1 && adj1 - c0 > (adj1 - adj0) * 0.2 && xi1 <= ci0)
			{
				xi0 = ci0;
				xi1 = xi0 + 1;
			}
			else if (c0 < adj0 && adj0 < c1 && c1 < adj1 && c1 - adj0 > (adj1 - adj0) * 0.2 && ci1 < xi0)
			{
				xi0 = ci1;
				xi1 = ci1 + 1;
			}
...
void SplashXPath::mergeSegments(int first)
{
	...
	while (in < length)
	{
		...
		for (i = in + 1; i < length; ++i)
		{
			...
			if (!joinAngleIsFlat(segs[prev].x0, segs[prev].y0, segs[i].x0, segs[i].y0, segs[i].x1, segs[i].y1))
				break;
...
#if USE_FIXEDPOINT
		if (seg->y0 == seg->y1 || seg->x0 == seg->x1 || !FixedPoint::divCheck(seg->x1 - seg->x0, seg->y1 - seg->y0, &seg->dxdy) || !FixedPoint::divCheck(seg->y1 - seg->y0, seg->x1 - seg->x0, &seg->dydx))
		{
			seg->dxdy = 0;
			seg->dydx = 0;
		}
#else
		if (splashAbs(seg->y1 - seg->y0) < 1e-200 || splashAbs(seg->x1 - seg->x0) < 1e-200)
		{
			seg->dxdy = 0;
			seg->dydx = 0;
		}
