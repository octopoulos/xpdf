    ...
    : QApplication(argc, argv)
{
	...
	if (zoomScaleFactor != 1)
	{
		...
		if (initialZoom > 0)
		{
			initialZoomStr = GString::format("{0:d}", (int)(initialZoom * zoomScaleFactor));
...
#endif
				openInNewWindow(qFileName, 1, "", rotateArg, passwordArg, fullScreen, "default");
			}
			else
			{
		...
		while (i < argc)
		{
			...
			if (viewers->getLength() > 0)
				ok = ((XpdfViewer*)viewers->get(0))
				         ->openInNewTab(qFileName, pg, dest, rotateArg, passwordArg, gFalse);
			else
				ok = openInNewWindow(qFileName, pg, dest, rotateArg, passwordArg, fullScreen);
		}
	}
	else
	{
...
XpdfViewer* XpdfApp::newWindow(GBool fullScreen, const char* remoteServerName, int x, int y, int width, int height)
{
...
GBool XpdfApp::openInNewWindow(QString fileName, int page, QString dest, int rotate, QString password, GBool fullScreen, const char* remoteServerName)
{
	XpdfViewer* viewer;

	viewer = XpdfViewer::create(this, fileName, page, dest, rotate, password, fullScreen);
	if (!viewer)
		return gFalse;
	viewers->append(viewer);
	if (remoteServerName)
...
void XpdfApp::saveSession(const char* id, GBool interactive)
{
	...
	if (!out)
	{
		if (interactive)
		{
			GString* msg = GString::format("Couldn't write the session file '{0:t}'", path);
...
void XpdfApp::loadSession(const char* id, GBool interactive)
{
	...
	if (!in)
	{
		if (interactive)
		{
			GString* msg = GString::format("Couldn't read the session file '{0:t}'", path);
			QMessageBox::warning(NULL, "Xpdf Error", msg->getCString());
			delete msg;
		}
		delete path;
	...
	if (viewers->getLength() == 1 && ((XpdfViewer*)viewers->get(0))->isEmpty())
